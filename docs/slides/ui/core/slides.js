...
var currentTheme;
var preferredTheme = 'default';
var preferredThemeTitle = 'Default S5 theme'
var alternateThemes = new Array();
var alternateThemeTitles = new Array();
...
function toggle() {
	...
	if (!slides.disabled) {
        ...
        setActiveStyleSheet(currentTheme);
		s5mode = true;
		fontScale();
		for (var n = 0; n < smax; n++) {
			var slide = slideColl[n];
...
function showHide(action) {
    var obj;
	var objs = GetElementsWithClassName('*','hideme');
...
function createStyleSwitcher() {
    ...
    if (!areAlternativeStyles()) return "";

    var activeStyle = getActiveStyleSheet();
    var links = document.getElementsByTagName('link');
    var switcher = '<select id="styleList" onchange="setActiveStyleSheet(this.value)">';
    for (var i = 0; i < links.length; i++) {
        link = links[i];
        if (link.getAttribute("rel").indexOf("style") != -1
          && link.getAttribute("title")) {
            title = link.getAttribute("title");
            switcher += '<option value="' + title + '">' +
                title + (title == activeStyle ? " (*)" : "") + '</option>';
        }
    }
    switcher += '<\/select>';
    return switcher;
...
function createControls() {
	...
	if (controlVis == 'hidden') {
    	...
    	addClass(document.getElementById('jumplist'), 'hideme');
    	addClass(document.getElementById('styleList'), 'hideme');    
	}

}

function fontScale() {  // causes layout problems in FireFox that get fixed if browser's Reload is used; same may be true of other Gecko-based browse
...
function addPreferredThemeStyle(head) {
    var link = document.createElement('link');
    link.setAttribute('rel', 'stylesheet');
    var url = "ui/themes/" + preferredTheme + "/hook.css";
    link.setAttribute('href', url);
    link.setAttribute('type', 'text/css');
    link.setAttribute('media', 'screen');
    link.setAttribute('title', preferredThemeTitle);
    head.appendChild(link);
}

function addAlternateThemeStyle(head, theme, title) {
    var link = document.createElement('link');
    link.setAttribute('rel', 'alternate stylesheet');
    var url = "ui/themes/" + theme + "/hook.css";
    link.setAttribute('href', url);
    link.setAttribute('type', 'text/css');
    link.setAttribute('media', 'screen');
...
function createThemeStyles() {
    ...
    addPreferredThemeStyle(head);
	for (var i = 0; i < alternateThemes.length; i++) {
        theme = alternateThemes[i];
        title = alternateThemeTitles[i];
        addAlternateThemeStyle(head, theme, title);
    }
}
...
function defaultCheck() {
	...
	for (var i = 0; i< allMetas.length; i++) {
        ...
        if (allMetas[i].name == 'preferredTheme') {
            preferredTheme = content;
        }
        if (allMetas[i].name == 'preferredThemeTitle') {
            preferredThemeTitle = content;
        }
        if (allMetas[i].name.indexOf('alternateTheme') != -1) {
            if (allMetas[i].name.indexOf('alternateThemeTitle') != -1) {
                alternateThemeTitles[alternateThemeTitles.length] = content;   
            } else {
                alternateThemes[alternateThemes.length] = content;   
            }
        }
	}
}
